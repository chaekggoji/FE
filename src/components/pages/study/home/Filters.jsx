  ...
  return (
    <div className='flex gap-x-4'>
      ...
      <div className='relative text-left'>
        ...
        {isDurationOpen && (
          <div className='absolute text-2xl left-0 right-0 bg-white border rounded-lg shadow-lg z-20'>
            {DURATION_FILTERS.map((option) => (
              <div
                key={option.value}
                className='p-4 hover:bg-primary-200 cursor-pointer'
                onClick={() => {
                  setDuration(option.label); // 클릭 시 label로 설정
      ...
      <div className='relative text-left'>
        ...
        {isCategoryOpen && (
          <div className='absolute text-2xl left-0 right-0 bg-white border rounded-lg shadow-md z-20'>
            {BOOK_CATEGORIES.map((option) => (
              <div
                key={option.value}
                className='p-4 hover:bg-primary-200 cursor-pointer'
                onClick={() => {
                  setCategory(option.label); // 클릭 시 label로 설정
