...
import arrowIcon from '/src/assets/icons/icon_arrow_right_24.svg'
            ...
            {index === 0 ? (
              ...
              <div className='w-full flex flex-row justify-between items-center'>
                ...
                <div className='flex flex-col text-right w-[55%]'>
                  <h1 className='[-webkit-text-stroke:2px_black] text-[5rem] text-primary-200'>책꼬지가 뭔가요?</h1>
              ...
              <>
                { /* 책꼬지 활용법 */}
                <h1 className='text-white [-webkit-text-stroke:2px_black] text-[5rem] mb-6'><span className='text-secondary-300'>책꼬지</span> 활용하는 법</h1>
                { /* 탭 영역 */}
                ...
                { /* 방법 영역 */}
                <div className='w-full max-w-[1200px] h-[50%] min-h-[400px] bg-[#FFF5E4] p-8 rounded-b-lg text-left flex flex-col justify-center gap-16 pl-10'>
                  {activeTab === 1 ? (
                    <>
              ...
              <div className='w-full flex flex-row justify-between items-center'>
                ...
                { /* 오른쪽 텍스트 영역 */}
                <div className='flex flex-col text-left w-[55%]'>
                  <h1 className='[-webkit-text-stroke:2px_black] -webkit-text-stroke: 2px black text-[5rem] text-right text-primary-200'>토론과 메모로 인증해요!</h1>
              ...
              <>
                ...
                <div className='w-full max-w-[1000px] flex flex-col gap-4'>
                  {faqData.map((faq, i) => (
                    <div
                      key={i}
                      className='bg-[#FFF5E4] rounded-lg shadow-xl text-left text-[3rem]'
                    >
                      { /* 질문 부분 */}
                      <button
                      ...
                      >
                        {faq.question}
                        <span className='text-[2rem] text-primary-300'>{openFaq === i ? '▲' : '▼'}</span>
                      </button>
              ...
              <>
                ...
                <div className='flex items-center gap-4'>
                  ...
                  { /* ✅ 로그인 페이지 이동 버튼 */}
                  <button
                    className='p-4 ml-6 rounded-lg bg-primary-300 hover:bg-primary-400 transition-all flex items-center justify-center'
                    onClick={() => navigate('/login')}
                  >
                    <img src={arrowIcon} alt="arrow icon" className="w-[2rem] h-[2rem] filter brightness-0 invert" />
                  </button>
                </div>
              </>
            ) : (
              // 추후 추가될 섹션을 위해 놔둠
              <div className='text-[5rem] w-full text-center'>{section.title}</div>
            )}
          </div>
        </section>
      ))}
